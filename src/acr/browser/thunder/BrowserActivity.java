...
public class BrowserActivity extends Activity implements BrowserController {
						...
						.setNegativeButton(getResources().getString(R.string.action_delete),
								new DialogInterface.OnClickListener() {
									...
									public void onClick(DialogInterface dialog, int which) {
										mBookmarkManager.deleteBookmark(url);
	...
	public synchronized void editBookmark(String url) {
		...
		picker.setPositiveButton(getResources().getString(R.string.action_ok),
				new DialogInterface.OnClickListener() {
					...
					@Override
					public void onClick(DialogInterface dialog, int which) {
						mBookmarkList.get(loc).setTitle(getTitle.getText().toString());
						mBookmarkList.get(loc).setUrl(getUrl.getText().toString());
			...
			for (int n = 0; n < mWebViewList.size(); n++) {
				if (mWebViewList.get(n).getUrl() != null) {
					if (!mWebViewList.get(n).getUrl().equals(mHomepage)
							&& !mWebViewList.get(n).getUrl().startsWith(Constants.FILE)) {
	...
	public boolean getSystemBrowser() {
		...
		mEditPrefs.putBoolean(PreferenceConstants.SYSTEM_BROWSER_PRESENT, browserFlag);
		mEditPrefs.commit();
		return browserFlag;
	}
			...
			public boolean onMenuItemClick(MenuItem item) {
				switch (item.getItemId()) {
					...
					case R.id.action_add_bookmark:
						if (!mCurrentView.getUrl().startsWith(Constants.FILE)) {
							mBookmarkManager.addBookmark(new HistoryItem(mCurrentView.getUrl(),
									mCurrentView.getTitle()));
							mSearchAdapter.refreshBookmarks();
						}
						return true;
	...
	public void initializeTor() {
		...
		try {
			...
			WebkitProxy.setProxy("acr.browser.thunder.BrowserApp", getApplicationContext(), host,
					port);
		} catch (Exception e) {
	...
	private synchronized void initializeSearchSuggestions(final AutoCompleteTextView getUrl) {
